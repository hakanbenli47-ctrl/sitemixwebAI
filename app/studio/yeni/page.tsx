...
import { ArrowLeft, ArrowRight, Check, ChevronRight, CirclePlus, Layers3, Palette, Sparkles } from "lucide-react";
import {
  YENI_SEKTORLER,
...
export default function YeniProjeSayfasi() {
  ...
  function projeyiOlustur() {
    ...
    const projeSlug = slugOlustur(slug || firmaAdi);
    const proje: ProjeVerisi = {
      id: kimlikOlustur(), firmaAdi: firmaAdi.trim(), sektor, sektorAdi: tanim.ad, siteTipi: "cok-sayfa", telefon: telefon.trim(), whatsapp: (whatsapp || telefon).trim(), eposta: eposta.trim(), adres: adres.trim(), sehir: sehir.trim(), ilce: ilce.trim(), hizmetBolgesi: hizmetBolgesi.trim(), slug: projeSlug, tema, sektorTemasi: tema, sayfalar: siteSayfalariOlustur(sektor), isletmeAlanlari: alanlar, hizmetler, ekOzellikler: ozellikler, medyalar: medyaAlanlariOlustur(sektor), siteIcerigi: { ...icerik, hizmetler, ozellikler }, yeniSistemMi: true, projeSurumu: 1, sablonSurumu: 10, seoBaslik: `${firmaAdi.trim()} | ${tanim.ad}`, seoAciklama: icerik.heroAciklama, seoKelimeler: [firmaAdi.trim(), tanim.ad, sehir.trim(), ilce.trim()].filter(Boolean), gorselAlanlariHazirlandiMi: true, olusturulmaTarihi: tarih, guncellenmeTarihi: tarih,
    };
    try {
  ...
  return (
    <main className={styles.sayfa}>
      ...
      {adim === 4 && <section className={styles.temaYerlesimi}>{tanim.temalar.map((kayit) => <button key={kayit.id} type="button" className={tema === kayit.id ? styles.seciliTema : ""} onClick={() => setTema(kayit.id)}><div className={styles.renkler}>{kayit.renkler.map((renk) => <i key={renk} style={{ background: renk }} />)}</div><span>{kayit.id.replace("tema-", "0")}</span><h2>{kayit.ad}</h2><p>{kayit.aciklama}</p><small>{tema === kayit.id ? "SEÇİLDİ" : "TEMAYI SEÇ"}</small></button>)}<div className={styles.medyaNotu}><Sparkles /><div><strong>Görsel alanları kapalı başlayacak.</strong><p>Düzenleme ekranında istediğiniz alanı açıp bir kez görsel yükleyebilirsiniz. Açılmayan veya görsel yüklenmeyen alan sitede hiç görünmez.</p></div></div></section>}
